var Settings = {
  token: "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..reaAdK4uUqvGcDghQTmXtbsHR4mX9Hcinwwg4_uqwfQ",
  basepath: "/wayfinding-demo", // in case of change it's required to change that also at ./src/environments and ./angular.json files
  port: "6006", // in case of change it's required to change that also at ./src/environments
  proximi_api: 'https://api.proximi.fi',
  geo_version: 'v5'
};

module.exports = Settings;
