var Settings = {
  token: "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..SpmhvgXw0fbjRhebvqf9L_8II4V6PV-4Xe4DpEs39Zo",
  basepath: "/maria01-v5", // in case of change it's required to change that also at ./src/environments and ./angular.json files
  port: "6004", // in case of change it's required to change that also at ./src/environments
  proximi_api: 'https://api.proximi.fi',
  geo_version: 'v5'
};

module.exports = Settings;
